...
pass. Both final hosted noncrank batches now pass too, completing all 26
controls in both viewer contexts. The current-root demonstration refresh
also passes: all six sequences replay twice with matching complete snapshots
and outcomes. Direct-operation tasks 6.1–6.6 are complete (17/23 overall).
Whole-machine tasks 1.3 and 5.1–5.5 remain open; the umbrella is not archived.
The evidence below this current checkpoint retains its historical scope.
...
- [x] 6.5 Bind every input to its actual visible part; test real pointer interactions in standalone and ordinary hosted viewers, including wrong-order attempts and partial motion, and inspect rest/moving/interlock snapshots. All 26 controls pass independent pointers in both contexts on the adopted 216-coordinate production export (manifest 68f16a67…, API27 bundle 4e458033…). The standalone index revalidates 22 individually passed interrupted-scan cases plus four terminal targeted probes without relabeling the interrupted run. Hosted batches A/B and the crank gate validate the exact 24+2 control set with identical initial banks and normal timestep. Fresh wrong-order lever gestures, nine-case full-bank Python/browser parity, partial crank, full part-button turn, loop end poses and inspected captures are pinned in simulation/docs/clearing-loop-replacement-2026-09-23.md. This is the named interaction contract, not every action permutation or whole-machine geometry acceptance.
- [x] 6.6 Replace obsolete calculator-page instructions and demos, update the simulation README and acceptance evidence, and report remaining whole-machine geometric gaps separately from the control migration. The current README describes the actual part controls, explicit preparation, simulation-only loop mounting, API27 requirement and performance limits; legacy posed material is marked historical. All six separate physical-action demonstrations replay twice on the adopted 216-coordinate root (2/2 unchanged tests, 1,919.092 seconds, exit zero), with complete snapshots/outcomes equal. The mounting, standalone interaction, demonstration and reconciliation records pin the final evidence. Whole-machine tasks 1.3 and 5.1–5.5 remain explicitly open; the umbrella change is not archived.
