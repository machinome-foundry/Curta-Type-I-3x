...
- [x] 5.2 Sync the new `result-carry-frame-clearance` capability and archive only this completed change; commit its implementation, tests, accepted evidence and completion record in the Curta repository, excluding generated artifacts and unrelated work. The four requirements are synced and strict-valid; implementation is committed as `3afcac9`. Only this completed change is archived at `openspec/changes/archive/2026-09-13-clear-result-carry-frame-contacts/`; this final documentation commit records closure. Generated artifacts remain ignored, and no integration is performed.
- [x] 5.3 Record the accepted correction commit in the open-run campaign handoff and resume the remaining transition, home-window and setup evidence work; report integration as pending unless separately authorized. Curta's acceptance/evidence records and the framework campaign roadmap name verified implementation `3afcac9`. Source-frame inventory and the pure-law retained-carry counterexample resume home-window preparation; geometry and outgoing-boundary checks remain open. Integration is pending and actual solid-node feature work still requires the pilot's go-ahead.
